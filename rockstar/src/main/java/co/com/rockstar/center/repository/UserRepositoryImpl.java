package co.com.rockstar.center.repository;

import co.com.rockstar.center.bean.User;
import co.com.rockstar.center.utiil.UseConnectionPool;
...
public class UserRepositoryImpl implements Repository<User> {
    ...
    public Integer saveObj(User user) throws SQLException {
        ...
        if (user.getId() != null && user.getId() > 0) {
            sql = "update Productos set nombre=?,descripcion=?,precio=?,cantidad=?,image=?)" + "where id=?";
        } else {
            sql = "insert into Productos(nombre,descripcion,precio,cantidad,image)" + "values (upper(?),upper(?),upper(?),upper(?),upper(?))";
        }
        try (Connection conn = UseConnectionPool.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
